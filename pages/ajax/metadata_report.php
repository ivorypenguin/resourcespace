...
if ($exiftool_fullpath==false)
   ...
   $command = $exiftool_fullpath . " -s -t -G --filename --exiftoolversion --filepermissions --NativeDigest --History --Directory " . escapeshellarg($image)." 2>&1";
    ...
    $command = $exiftool_fullpath . " -s -t -G1 --filename --exiftoolversion --filepermissions --NativeDigest --History --Directory " . escapeshellarg($image)." 2>&1";
    $report_originalG1 = run_command($command);
   ...
   $tmpfile = write_metadata($image, $ref);
    ...
    $command = $exiftool_fullpath . " -s -t -G1 --filename --exiftoolversion --filepermissions --NativeDigest --History --Directory " . escapeshellarg($tmpfile)." 2>&1";
    $report_simulatedG1 = run_command($command);
    
    # Remove the temporary file.
   unlink($tmpfile);
  $fields_simulatedG1 = explode("\n", $report_simulatedG1);
    # Process the report of the simulated download.
    $stag_groups=array();
        for ($n=0;$n<count( $fields_simulatedG1);$n++){
                $stag_value=explode("\t", $fields_simulatedG1[$n]);
                $stag_groups[trim(strtolower($stag_value[1]))]=trim(strtolower($stag_value[0]));
        }
    ...
    foreach ($fields_simulated as $field_simulated)
        ...
        if (count($tag_value)==3)
            ...
            $results_simulated[$i]["group1"]=$stag_groups[$results_simulated[$i]["tag"]];
   ...
   $write_to = get_exiftool_fields($resource_type); # Returns an array of exiftool tags for the particular resource type, which are basically RS resource fields with an 'exiftool field' set.
    ...
    # Work out the write status.
        ...
        echo "<tr><td colspan=\"6\">".$lang['resourcetype'].": ".$type_name."</td></tr>";
        echo "<tr><td colspan=\"6\">".$lang['existing_tags']."</td></tr>";
        echo "<tr><td width=\"150\">".$applicationname."</td><td width=\"50\">".$lang['group']."</td><td width=\"50\">".$lang['group']."1</td><td width=\"150\">".$lang['exiftooltag']."</td><td>".$lang['embeddedvalue']."</td><td>$write_status</td></tr>";
   ...
   $fields = explode("\n", $report_original);
	...
	$tag_groups=array();
	for ($n=0;$n<count($fieldsG1);$n++){
		$tag_value=explode("\t",$fieldsG1[$n]);
		$tag_groups[trim(strtolower($tag_value[1]))]=trim(strtolower($tag_value[0]));
	}
print_r($tag_groups);
    ...
    for ($n=0;$n<count($fields);$n++)
            ...
            # Check if the tag is mapped to an RS resource field.
                        if(isset($resourcefields[$tag]['ref']) || isset($resourcefields[$group.":".$tag]['ref']) || isset($resourcefields[$group1.":".$tag]['ref']))
                                {
                 # Work out the RS resource field ref and title for the tag, set the listed status of the tag.
                    ...
                    $resourcefields[$group.":".$tag]['listed'] = true;
                                ...
                                elseif (isset($resourcefields[$group1.":".$tag]['ref']))
                    {
                                        $RS_field_ref=$resourcefields[$group1.":".$tag]['ref'];
                    $resourcefields[$group1.":".$tag]['listed'] = true;
                    }


                                $RS_field_info=get_field($RS_field_ref);
                                $RS_field_name=$RS_field_info['title'];
                                $RS_field_mapping=str_replace(",",",<wbr>",$RS_field_info['exiftool_field']);
                                $RS_field_type_name=get_resource_type_name($RS_field_info['resource_type']);
 $RS_field_order_by=$RS_field_info['order_by'];
                                $RS_field_tab=$RS_field_info['tab_name'];
                                echo "<td>". $RS_field_ref . " - " . lang_or_i18n_get_translated($RS_field_name, "fieldtitle-") . "<br />".$RS_field_mapping."<br/>".$RS_field_type_name." ".$RS_field_order_by." ".$RS_field_tab."</td><td>$group</td><td>$group1</td><td>$tag $tagprops</td>";

                                }
                        else
                                {
                # Not an RS resource field; display exiftool group, tag and properties.
                                echo "<td></td><td>$group</td><td>$group1</td><td>$tag $tagprops</td>";
                                }
   ...
   if ($exiftool_write&&$file_writability)
        {
        echo "<tr><td colspan=\"6\">" . $lang['new_tags'] . "</td></tr>";
        echo "<tr><td width=\"150\">".$applicationname."</td><td width=\"50\">".$lang['group']."</td><td width=\"50\">".$lang['group']."1</td><td width=\"150\">".$lang['exiftooltag']."</td><td>".$lang['embeddedvalue']."</td><td>$write_status</td></tr>";
       ...
       foreach ($results_simulated as $result_simulated)
            ...
            $group1 = $result_simulated["group1"];
  ...
  # Display the RS resource field ref, title, exiftool group, tag and properties.
               echo "<td>". str_replace(array('%ref%', '%name%'), array($RS_field_ref, $RS_field_name), $lang['field_ref_and_name']) . "</td><td>$group</td><td>$group1</td><td>$tag $tagprops</td>";
