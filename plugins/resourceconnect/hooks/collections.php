...
function HookResourceconnectCollectionsThumblistextra()
	...
	foreach ($thumbs as $thumb)
		...
		<a href="../plugins/resourceconnect/pages/view.php?url=<?php echo urlencode($thumb["url"]) ?>&k=<?php echo getval("k","") ?>&col=<?php echo $usercollection ?>"><img border=0 src="<?php echo $thumb["thumb"] ?>" class="CollectImageBorder" /></a></td> 
		</tr></table> 

		<div class="CollectionPanelInfo"><a href="../plugins/resourceconnect/pages/view.php?url=<?php echo urlencode($thumb["url"]) ?>&k=<?php echo getval("k","") ?>&col=<?php echo $usercollection ?>"><?php echo tidy_trim(i18n_get_translated($thumb["title"]),15) ?></a>&nbsp;</div> 

		<div class="CollectionPanelInfo"> 
		<a href="collections.php?resourceconnect_remove=<?php echo $thumb["ref"] ?>&nc=<?php echo time() ?>">x <?php echo $lang["action-remove"]?></a></div>
		</div>
		<?php
		}
	}
