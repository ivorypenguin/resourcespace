...
// This file has been modified for ResourceSpace to include the options 'selectOnlyFiles' and 'allowedExtensions' and to exclude .DS_Store from displaying
/**
...
$selectOnlyFiles = ( isset($_POST['selectOnlyFiles']) && $_POST['selectOnlyFiles'] == 'true' ) ? true : false;
...
if( file_exists($postDir) ) {
	...
	if( count($files) > 2 ) { // The 2 accounts for . and ..
		...
		foreach( $files as $file ) {
			...
			if( file_exists($postDir . $file) && $file != '.' && $file != '..' && $file !='.DS_Store' && ($allowedExtensions=='' || ($allowedExtensions!='' && (strpos($allowedExtensions,$ext)!==false) || $ext==$file))) {
				if( is_dir($postDir . $file) && (!$onlyFiles || $onlyFolders) )
					echo "<li class='directory collapsed'>" . (!$selectOnlyFiles ? $checkbox : '' ) ."<a rel='" .$htmlRel. "/'>" . $htmlName . "</a></li>";
