...
function CentralSpaceLoad (anchor,scrolltop)
	...
	context="";
	CentralSpace=jQuery('#CentralSpace');// for ajax targeting top div
	if (jQuery("#CollectionDiv").length==0 && jQuery("#CollectionFrameless").length==0){ 
		CentralSpace=top.main.jQuery('#CentralSpace');
		context="parent.main.document";
		altcontext="top.main";
	}
	...
	* $ajax_collections also removes this condition
	...
	if (scrolltop==true) {jQuery('html, body'.altcontext).animate({scrollTop:0}, 'fast');};
	...
	CentralSpace.load(url, function ()
		{
		// Load completed
...
function CentralSpaceShowLoading()
	{
	ClearLoadingTimers();
	loadingtimers.push(window.setTimeout("jQuery('#CentralSpace').fadeTo('fast',0.7);jQuery('#LoadingBox').fadeIn('fast');",1500));
	}
