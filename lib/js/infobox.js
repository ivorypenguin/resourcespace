...
function InfoBoxMM(event)
	...
	var iscollection = (event.target.getAttribute('class')=='CollectImageBorder');
	if (iscollection)
		{
    ...
    if ((InfoBoxWaiting==false) && (InfoBoxVisible==false))
    	...
    	InfoBoxTimer=window.setTimeout("InfoBoxAppear('" + iname + "', '" + iiname + "', " + iscollection + ")",800);
	    InfoBoxWaiting=true;
		}
    }
...
function InfoBoxAppear(boxName, innerBoxName, iscollection)
	{
	...
	if ((InfoBoxWaiting) && (InfoBoxRef!=0))
        ...
        //new Effect.Opacity(boxName, {duration:0.3, from:1, to:0.8});
