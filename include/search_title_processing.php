...
if ($search_titles)
	...
	if (substr($search,0,11)=="!collection"){
		if ($k=="" && !checkperm("b")){$search_title_links='<a onClick="return CollectionDivLoad(this);" href="'.$baseurl_short.'pages/collections.php?collection='.$collectiondata["ref"].'" target="collections">&gt;&nbsp;'.$lang["selectcollection"].'</a>&nbsp;&nbsp;';}
		if (count($result)!=0 && $k==""&&$preview_all){$search_title_links.='<a onClick="return CentralSpaceLoad(this,true);" href="'.$baseurl_short.'pages/preview_all.php?ref='.$collectiondata["ref"].'&order_by='.$order_by.'&sort='.$sort.'&archive='.$archive.'&k='.$k.'">&gt;&nbsp;'.$lang['preview_all'].'</a>';}
		$search_title.='</div>';
