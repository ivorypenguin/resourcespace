...
if ($search_titles)
	...
	if (substr($search,0,11)=="!collection"){
		...
		if (count($result)!=0 && $k==""&&$preview_all){$search_title_links.='<a onClick="CentralSpaceLoad(this,true);" href="'.$baseurl_short.'pages/preview_all.php?ref='.$collectiondata["ref"].'&order_by='.$order_by.'&sort='.$sort.'&archive='.$archive.'&k='.$k.'">&gt;&nbsp;'.$lang['preview_all'].'</a>';}
		$search_title.='</div>';
